...
The following is a list of all the error codes that can be returned by the Rooftop API. You may also refer to [RFC2616](http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html) for a list of http error codes. Make sure to check the error message for more details.
...
| `NotInitialized`	               |  109 | You must call Rooftop.initialize before using the Rooftop library. Check the Quick Start guide for your platform. |
| `ObjectTooLarge`	               |  116 | The object is too large. Rooftop Objectss have a max size of 128 kilobytes. |
...
| `ClientPushDisabled`	           |  115 | Client-initiated push is not enabled. Check your Rooftop app's push notification settings. |
| `RestPushDisabled`	             |  115 | REST-initiated push is not enabled. Check your Rooftop app's push notification settings. |
...
| Name                             | Code | Description                                                   |
|----------------------------------|------|---------------------------------------------------------------|
| `InvalidFileName`	               |  122 | An invalid filename was used for Rooftop File. A valid file name contains only a-zA-Z0-9_. characters and is between 1 and 128 characters. |
...
| `UsernameSigninDisabled`         |  252 | Authentication by username and password is not supported for this application. Check your Rooftop app's authentication settings. |
| `AnonymousSigninDisabled`        |  252 | Anonymous users are not supported for this application. Check your Rooftop app's authentication settings. |
| `FacebookSigninDisabled`         |  252 | Authentication by Facebook is not supported for this application. Check your Rooftop app's authentication settings. |
| `TwitterSigninDisabled`          |  252 | Authentication by Twitter is not supported for this application. Check your Rooftop app's authentication settings. |
...
| Name                             | Code | Description                                                   |
|----------------------------------|------|---------------------------------------------------------------|
| `ConnectionFailed`	             |  100 | The connection to the Rooftop servers failed.                   |
...
| `FileReadError `                 |  601 | Unable to read input for a Rooftop File on the client. |
...
| Name                             | Code | Description                                                   |
|----------------------------------|------|---------------------------------------------------------------|
| `RequestTimeout`                 |  124 | The request was slow and timed out. Typically this indicates that the request is too expensive to run. You may see this when a Cloud function did not finish before timing out, or when a `Rooftop.Cloud.httpRequest` connection times out.  |
...
| `RequestLimitExceeded`	         |  155 | This application has exceeded its request limit (legacy Rooftop.com apps only). |
| `TemporaryRejectionError`	       |  159 | An application's requests are temporary rejected by the server (legacy Rooftop.com apps only). |
| `DatabaseNotMigratedError`       |  428 | You should migrate your database as soon as possible (legacy Rooftop.com apps only). |
{: .docs_table}
...
| Name                             | Code | Description                                                     |
|----------------------------------|------|-----------------------------------------------------------------|
| `OtherCause`	                   |   -1 | An unknown error or an error unrelated to Rooftop occurred.       |
